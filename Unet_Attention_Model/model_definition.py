...
def numpy_open_files_select_molecule_path(path, parent_dir=False):
	...
	N = torch.from_numpy(np.load(path + "for_labview_protein_/N_tensor.npy").astype(np.float32))
	...
	bfactors = torch.from_numpy(np.load(path+"for_labview_protein_/bfactors_tensor.npy").astype(np.float32))
	...
	buriedness = torch.from_numpy(np.load(path+"for_labview_protein_/buriedness_tensor.npy").astype(np.float32))
	...
	charge = torch.from_numpy(np.load(path+"for_labview_protein_/charge_tensor.npy").astype(np.float32))
	# charge = charge
	charge = mean_max_scaler(non_zeros, charge)
	charge[zeros] = nzv
	charge = charge.unsqueeze(-1)

	radius = torch.from_numpy(np.load(path+"for_labview_protein_/radius_tensor.npy").astype(np.float32))
	# radius = radius 
	radius = mean_max_scaler(non_zeros, radius)
	radius[zeros] = nzv
	radius = radius.unsqueeze(-1)

	hbdon = torch.from_numpy(np.load(path+"for_labview_protein_/hbdon_tensor.npy").astype(np.float32))
	# hbdon = hbdon 
	hbdon = mean_max_scaler(non_zeros, hbdon)
	hbdon[zeros] = nzv
	hbdon = hbdon.unsqueeze(-1)

	hbacc = torch.from_numpy(np.load(path+"for_labview_protein_/hbac_tensor.npy").astype(np.float32))
	# hbacc = hbacc 
	hbacc = mean_max_scaler(non_zeros, hbacc)
	hbacc[zeros] = nzv
	hbacc = hbacc.unsqueeze(-1)

	sasa = torch.from_numpy(np.load(path+"for_labview_protein_/sasa_tensor.npy").astype(np.float32))
	# sasa = sasa 
	...
	# target feature
	target=torch.from_numpy(np.load(path+"for_labview_pocket_/N_tensor_new_pocket.npy"))
